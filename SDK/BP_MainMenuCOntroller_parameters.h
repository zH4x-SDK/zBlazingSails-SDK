#pragma once

#include "../SDK.h"

// Name: BS, Version: 1.536.0

#ifdef _MSC_VER
	#pragma pack(push, 0x8)
#endif

namespace SDK
{
//---------------------------------------------------------------------------
// Parameters
//---------------------------------------------------------------------------

// Function BP_MainMenuCOntroller.BP_MainMenuCOntroller_C.UserConstructionScript
struct ABP_MainMenuCOntroller_C_UserConstructionScript_Params
{
};

// Function BP_MainMenuCOntroller.BP_MainMenuCOntroller_C.InpActEvt_LeftMouseButton_K2Node_InputKeyEvent_2
struct ABP_MainMenuCOntroller_C_InpActEvt_LeftMouseButton_K2Node_InputKeyEvent_2_Params
{
	struct FKey                                        Key;                                                      // (BlueprintVisible, BlueprintReadOnly, Parm)
};

// Function BP_MainMenuCOntroller.BP_MainMenuCOntroller_C.InpActEvt_LeftMouseButton_K2Node_InputKeyEvent_1
struct ABP_MainMenuCOntroller_C_InpActEvt_LeftMouseButton_K2Node_InputKeyEvent_1_Params
{
	struct FKey                                        Key;                                                      // (BlueprintVisible, BlueprintReadOnly, Parm)
};

// Function BP_MainMenuCOntroller.BP_MainMenuCOntroller_C.ReceiveBeginPlay
struct ABP_MainMenuCOntroller_C_ReceiveBeginPlay_Params
{
};

// Function BP_MainMenuCOntroller.BP_MainMenuCOntroller_C.InpAxisKeyEvt_MouseX_K2Node_InputAxisKeyEvent_1
struct ABP_MainMenuCOntroller_C_InpAxisKeyEvt_MouseX_K2Node_InputAxisKeyEvent_1_Params
{
	float                                              AxisValue;                                                // (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData)
};

// Function BP_MainMenuCOntroller.BP_MainMenuCOntroller_C.ReceiveTick
struct ABP_MainMenuCOntroller_C_ReceiveTick_Params
{
	float                                              DeltaSeconds;                                             // (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData)
};

// Function BP_MainMenuCOntroller.BP_MainMenuCOntroller_C.CreateAndAddWaterCullingVolume
struct ABP_MainMenuCOntroller_C_CreateAndAddWaterCullingVolume_Params
{
	class ABP_BoatMeshBase_C*                          BoatMeshBaseRef;                                          // (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData)
};

// Function BP_MainMenuCOntroller.BP_MainMenuCOntroller_C.ExecuteUbergraph_BP_MainMenuCOntroller
struct ABP_MainMenuCOntroller_C_ExecuteUbergraph_BP_MainMenuCOntroller_Params
{
	int                                                EntryPoint;                                               // (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData)
};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
